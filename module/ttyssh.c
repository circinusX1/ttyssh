 ...
 *
 * See the Documentation/serial/ttyssh.rts for more details.
 *
...
#define ttyssh_WAKES_PER_SEC	1000
#else
#define ttyssh_WAKES_PER_SEC	HZ
#endif

#define ttyssh_XBUFSIZE	32
...
struct ttyssh_intf {
     ...
     */
    unsigned char xmitbuf[ttyssh_XBUFSIZE];
    struct circ_buf buf;
...
#define circ_sbuf_next(idx) (((idx) + 1) % ttyssh_XBUFSIZE)

static struct ttyssh_intf *ttyssh_port_to_intf(struct uart_port *port)
{
    return container_of(port, struct ttyssh_intf, port);
}

static unsigned int ttyssh_tx_empty(struct uart_port *port)
{
    struct ttyssh_intf *intf = ttyssh_port_to_intf(port);
 ...
 * the time.
 */
static void ttyssh_null_modem_lock_irq(struct ttyssh_intf *intf)
...
static void ttyssh_null_modem_unlock_irq(struct ttyssh_intf *intf)
 ...
 */
static void _ttyssh_set_modem_lines(struct ttyssh_intf *intf,
                    unsigned int mask,
                    unsigned int new_mctrl)
{
 ...
 */
static void ttyssh_handle_null_modem_update(struct ttyssh_intf *intf)
{
    ...
    _ttyssh_set_modem_lines(intf, NULL_MODEM_MCTRL, mctrl);
}

static void ttyssh_set_null_modem(struct ttyssh_intf *intf, bool val)
{
    ttyssh_null_modem_lock_irq(intf);
    ...
    ttyssh_handle_null_modem_update(intf);

out_unlock:
    ttyssh_null_modem_unlock_irq(intf);
}

static void ttyssh_set_modem_lines(struct ttyssh_intf *intf,
                      unsigned int mask,
                      unsigned int new_mctrl)
{
    mask &= LOCAL_MCTRL;
    ...
    _ttyssh_set_modem_lines(intf, mask, new_mctrl);
...
{
    struct ttyssh_intf *intf = (void *) data;
...
static void ttyssh_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
    struct ttyssh_intf *intf = ttyssh_port_to_intf(port);
...
static unsigned int ttyssh_get_mctrl(struct uart_port *port)
{
    struct ttyssh_intf *intf = ttyssh_port_to_intf(port);
    unsigned int rv;
...
static void ttyssh_stop_tx(struct uart_port *port)
{
    struct ttyssh_intf *intf = ttyssh_port_to_intf(port);
...
static void ttyssh_set_baud_rate(struct ttyssh_intf *intf,
                     unsigned int baud, unsigned int cflag)
{
    ...
    intf->div = ttyssh_WAKES_PER_SEC * bits_per_char;
    intf->bytes_per_interval = baud / intf->div;
    intf->per_interval_residual = baud % intf->div;
}

static void ttyssh_transfer_data(struct uart_port *port,
...
static unsigned int ttyssh_get_flag(struct ttyssh_intf *intf,
                       unsigned int *status)
{
...
static void ttyssh_set_flags(struct ttyssh_intf *intf,
                 unsigned int status)
{
    spin_lock_irq(&intf->port.lock);
    intf->flags |= status;
...
static void ttyssh_thread_delay(void)
{
#ifdef CONFIG_HIGH_RES_TIMERS
    ktime_t timeout;

    timeout = ns_to_ktime(1000000000 / ttyssh_WAKES_PER_SEC);
...
static int ttyssh_thread(void *data)
{
    struct ttyssh_intf *intf = data;
    struct ttyssh_intf *ointf = intf->ointf;
    ...
    while (!kthread_should_stop()) {
        ...
        if (ointf->tx_enabled && oport->state->xmit.buf)
             ...
             */
            ttyssh_transfer_data(oport, tbuf);
        ...
        if (intf->rx_enabled) {
            ...
            for (i = 0; i < to_send && !circ_sbuf_empty(tbuf);
                ...
                flag = ttyssh_get_flag(intf, &status);
                port->icount.rx++;
...
static void ttyssh_start_tx(struct uart_port *port)
{
    struct ttyssh_intf *intf = ttyssh_port_to_intf(port);
...
static void ttyssh_stop_rx(struct uart_port *port)
{
    struct ttyssh_intf *intf = ttyssh_port_to_intf(port);
...
static void ttyssh_break_ctl(struct uart_port *port, int break_state)
{
    struct ttyssh_intf *intf = ttyssh_port_to_intf(port);
    struct ttyssh_intf *ointf = intf->ointf;
...
static int ttyssh_startup(struct uart_port *port)
{
    struct ttyssh_intf *intf = ttyssh_port_to_intf(port);
    int rv = 0;
    struct circ_buf *cbuf = &port->state->xmit;
    ...
    intf->thread = kthread_run(ttyssh_thread, intf,
                   "%s%d", intf->threadname, port->line);
    if (IS_ERR(intf->thread)) {
        rv = PTR_ERR(intf->thread);
        intf->thread = NULL;
        pr_err("ttyssh: Could not start thread: %d", rv);
    } else {
        spin_lock_irqsave(&port->lock, flags);
        intf->tx_enabled = true;
        intf->rx_enabled = true;

        ttyssh_set_baud_rate(intf, 9600, CS8);
        spin_unlock_irqrestore(&port->lock, flags);
    }
...
static void ttyssh_shutdown(struct uart_port *port)
{
    struct ttyssh_intf *intf = ttyssh_port_to_intf(port);
    unsigned long flags;
...
static void ttyssh_release_port(struct uart_port *port)
{
}

static void
ttyssh_set_termios(struct uart_port *port, struct ktermios *termios,
              CONST_KTERMIOS struct ktermios *old)
{
    ...
    spin_lock_irqsave(&port->lock, flags);
    ttyssh_set_baud_rate(intf, baud, termios->c_cflag);
    intf->termios = *termios;
    spin_unlock_irqrestore(&port->lock, flags);
}

static int ttyssh_rs485(struct uart_port *port, RS485_HAS_TERMIOS
                ...
                unsigned long arg)
{
    ...
    case TIOCSERSREMNULLMODEM:
        ttyssh_set_null_modem(intf->ointf, !!arg);
        break;
    ...
    case TIOCSERSREMMCTRL:
        ...
        if (mask & ~LOCAL_MCTRL || arg & ~LOCAL_MCTRL)
            ...
            ttyssh_set_modem_lines(intf->ointf, mask, arg);
        break;
...
static unsigned int nr_pipe_ports = 1;
...
static void ttyssh_ctrl_append(char **val, int *left, char *n, bool enabled)
{
    int count;
...
static ssize_t ttyssh_ctrl_read(struct device *dev,
                   ...
                   char *buf)
{
    ...
    ttyssh_ctrl_append(&val, &left, "nullmodem", intf->do_null_modem);
    ttyssh_ctrl_append(&val, &left, "cd", mctrl & TIOCM_CAR);
    ttyssh_ctrl_append(&val, &left, "dsr", mctrl & TIOCM_DSR);
    ttyssh_ctrl_append(&val, &left, "cts", mctrl & TIOCM_CTS);
    ttyssh_ctrl_append(&val, &left, "ring", mctrl & TIOCM_RNG);
    ttyssh_ctrl_append(&val, &left, "dtr", mctrl & TIOCM_DTR);
    ttyssh_ctrl_append(&val, &left, "rts", mctrl & TIOCM_RTS);
    *val++ = '\n';

    return val - buf;
}

static ssize_t ttyssh_ctrl_write(struct device *dev,
                    struct device_attribute *attr,
                    const char *val, size_t count)
{
    ...
    if (flags)
        ttyssh_set_flags(intf, flags);
    if (nullmodem)
        ttyssh_set_null_modem(intf, nullmodem == '+');
    if (mctrl_mask)
        ttyssh_set_modem_lines(intf, mctrl_mask, mctrl);
...
static DEVICE_ATTR(ctrl, 0660, ttyssh_ctrl_read, ttyssh_ctrl_write);

static struct attribute *ttyssh_dev_attrs[] = {
    &dev_attr_ctrl.attr,
    NULL,
};
 ...
 */
static struct attribute_group ttyssh_dev_attr_group = {
    .attrs = ttyssh_dev_attrs,
};

static const struct uart_ops serialecho_ops = {
    .tx_empty =		ttyssh_tx_empty,
    .set_mctrl =		ttyssh_set_mctrl,
    .get_mctrl =		ttyssh_get_mctrl,
    .stop_tx =		ttyssh_stop_tx,
    .start_tx =		ttyssh_start_tx,
    .stop_rx =		ttyssh_stop_rx,
    .break_ctl =		ttyssh_break_ctl,
    .startup =		ttyssh_startup,
    .shutdown =		ttyssh_shutdown,
    .release_port =		ttyssh_release_port,
    .set_termios =		ttyssh_set_termios,
...
static struct uart_driver serialpipeb_driver = {
    .owner = THIS_MODULE,
    .driver_name = "vpoutput",
    .dev_name = "vpoutput"
};

#ifdef HAS_RS485_SUPPORTED
static const struct serial_rs485 ttyssh_rs485_supported = {
...
static DEFINE_MUTEX(ttyssh_num_mutex);
static DEFINE_IDR(ttyssh_echo_nums);
static DEFINE_IDR(ttyssh_pipe_nums);

static int ttyssh_alloc_intf(struct idr *idr,
                const char *name,
                const struct uart_ops *ops,
                int min_id, int max_id,
                struct ttyssh_intf **rintf)
{
    struct ttyssh_intf *intf;
    struct uart_port *port;
    int rv;

    intf = kzalloc(sizeof(*intf), GFP_KERNEL);
    if (!intf) {
        pr_err("ttyssh: Unable to alloc %s\n", name);
        return -ENOMEM;
    }
    port = &intf->port;
    ...
    if (idr) {
        ...
        if (id < 0) {
            pr_err("ttyssh: Unable to alloc id for %s: %d\n",
                   name, id);
            kfree(intf);
            return id;
        }
    ...
    if (!intf->pdev) {
        pr_err("ttyssh: Unable to alloc device for %s:%u: %d\n",
               name, port->line, rv);
        if (intf->idr)
    ...
    if (rv) {
        pr_err("ttyssh: Unable to add device for %s:%u: %d\n",
               name, port->line, rv);
    ...
    port->attr_group = &ttyssh_dev_attr_group;
    port->ops = ops;
...
static void ttyssh_free_intf(struct ttyssh_intf *intf)
{
    if (intf->driver)
...
static int ttyssh_add_port(int i, const char *name,
                  struct uart_driver *driver,
                  struct ttyssh_intf *intf)
{
    int rv;
    ...
    if (rv) {
        pr_err("ttyssh: Unable to add uart %s port %u: %d\n",
               name, i, rv);
        return rv;
    }
    intf->driver = driver;
...
struct ttyssh_dyninfo {
    struct idr nums;
};

static long ttyssh_echo_ioctl(struct file *file,
                 unsigned int cmd, unsigned long data)
{
    int rv = -ENOTTY;
    struct ttyssh_dyninfo *di = file->private_data;
    struct ttyssh_intf *intf;

    mutex_lock(&ttyssh_num_mutex);
    switch (cmd) {
    case ttyssh_ALLOC_ID:
        rv = ttyssh_alloc_intf(&ttyssh_echo_nums, "ttyEcho",
        ...
        rv = idr_alloc(&di->nums, intf,
                   ...
                   GFP_KERNEL);
        if (rv < 0) {
            ttyssh_free_intf(intf);
            break;
        }
        intf->ointf = intf;
        intf->do_null_modem = true;

        rv = ttyssh_add_port(-1, "echo", &serialecho_driver, intf);
        if (rv) {
    ...
    case ttyssh_FREE_ID:
        struct ttyssh_intf *intf = idr_remove(&di->nums, data);
    ...
    mutex_unlock(&ttyssh_num_mutex);

    return rv;
}

static int ttyssh_echo_dyn_release(int id, void *p, void *data)
{
    struct ttyssh_intf *intf = idr_find(&ttyssh_echo_nums, id);
...
static int ttyssh_echo_release(struct inode *ino, struct file *file)
{
    struct ttyssh_dyninfo *di = file->private_data;

    mutex_lock(&ttyssh_num_mutex);
    idr_for_each(&di->nums, ttyssh_echo_dyn_release, NULL);
    mutex_unlock(&ttyssh_num_mutex);
    idr_destroy(&di->nums);
    kfree(di);
    return 0;
...
static int ttyssh_echo_open(struct inode *ino, struct file *file)
{
    struct ttyssh_dyninfo *di = kzalloc(sizeof(*di), GFP_KERNEL);
...
const struct file_operations ttyssh_echo_misc_fops = {
        .owner = THIS_MODULE,
        .unlocked_ioctl = ttyssh_echo_ioctl,
        .open = ttyssh_echo_open,
        .release = ttyssh_echo_release
};

static struct miscdevice ttyssh_echo_misc = {
    .minor = MISC_DYNAMIC_MINOR,
    .name = "ttyEcho",
    .fops = &ttyssh_echo_misc_fops,
    .mode = 0660
};

static long ttyssh_pipe_ioctl(struct file *file,
                 unsigned int cmd, unsigned long data)
{
    int rv = -ENOTTY;
    struct ttyssh_dyninfo *di = file->private_data;
    struct ttyssh_intf *intfa, *intfb;

    mutex_lock(&ttyssh_num_mutex);
    switch (cmd) {
    case ttyssh_ALLOC_ID:
        rv = ttyssh_alloc_intf(&ttyssh_pipe_nums, "vpinput",
                      &serialpipea_ops, nr_pipe_ports,
                      nr_pipe_ports + nr_dyn_pipe_ports,
                      &intfa);
        if (rv)
            break;
        rv = ttyssh_alloc_intf(NULL, "vpoutput",
                      &serialpipeb_ops,
                      intfa->port.line, 0,
                      &intfb);
        if (rv) {
            ttyssh_free_intf(intfa);
            break;
        }
        ...
        intfa->port.rs485_config = ttyssh_rs485;
        intfb->port.rs485_config = ttyssh_rs485;
#ifdef HAS_RS485_SUPPORTED
        intfa->port.rs485_supported = ttyssh_rs485_supported;
        intfb->port.rs485_supported = ttyssh_rs485_supported;
#endif

        rv = ttyssh_add_port(-1, "pipea", &serialpipea_driver,
                    intfa);
        if (rv) {
        ...
        rv = ttyssh_add_port(-1, "pipeb", &serialpipeb_driver,
                    intfb);
        if (rv) {
            idr_remove(&di->nums, intfa->port.line);
            ttyssh_free_intf(intfb);
        ...
        ttyssh_set_null_modem(intfa, true);
        ttyssh_set_null_modem(intfb, true);
        break;
...
static int ttyssh_pipe_dyn_release(int id, void *p, void *data)
{
    struct ttyssh_intf *intf = idr_find(&ttyssh_pipe_nums, id);
...
static int ttyssh_pipe_release(struct inode *ino, struct file *file)
{
    struct ttyssh_dyninfo *di = file->private_data;

    mutex_lock(&ttyssh_num_mutex);
    idr_for_each(&di->nums, ttyssh_pipe_dyn_release, NULL);
    mutex_unlock(&ttyssh_num_mutex);
    idr_destroy(&di->nums);
    kfree(di);
    return 0;
...
static int ttyssh_pipe_open(struct inode *ino, struct file *file)
{
    struct ttyssh_dyninfo *di = kzalloc(sizeof(*di), GFP_KERNEL);
...
const struct file_operations ttyssh_pipe_misc_fops = {
        .owner = THIS_MODULE,
        .unlocked_ioctl = ttyssh_pipe_ioctl,
        .open = ttyssh_pipe_open,
        .release = ttyssh_pipe_release
};

static struct miscdevice ttyssh_pipe_misc = {
    .minor = MISC_DYNAMIC_MINOR,
    .name = "ttyPipe",
    .fops = &ttyssh_pipe_misc_fops,
    .mode = 0660
};

static int __init ttyssh_init(void)
{
    unsigned int i;
    int rv;
    ...
    if (rv) {
        pr_err("ttyssh: Unable to register echo driver.\n");
        goto out;
    }
    ...
    if (rv) {
        pr_err("ttyssh: Unable to register pipe a driver.\n");
        goto out_unreg_echo;
    }
    ...
    if (rv) {
        pr_err("ttyssh: Unable to register pipe b driver.\n");
    ...
    for (i = 0; i < nr_echo_ports; i++) {
        ...
        rv = ttyssh_add_port(i, "echo", &serialecho_driver, intf);
        if (rv) {
            ttyssh_free_intf(intf);
            break;
        }
    ...
    for (i = 0; !rv && i < nr_pipe_ports; i++) {
        struct ttyssh_intf *intfa;
        struct ttyssh_intf *intfb;
        ...
        rv = ttyssh_add_port(i, "pipea", &serialpipea_driver, intfa);
        if (rv) {
            ttyssh_free_intf(intfb);
            ttyssh_free_intf(intfa);
            break;
        ...
        rv = ttyssh_add_port(i, "pipeb", &serialpipeb_driver, intfb);
        if (rv) {
            ttyssh_free_intf(intfb);
            ttyssh_free_intf(intfa);
            break;
    ...
    rv = misc_register(&ttyssh_echo_misc);
    if (rv) {
        pr_err("ttyssh: Error registering dynamic echo device: %d\n",
               rv);
    }

    rv = misc_register(&ttyssh_pipe_misc);
    if (rv) {
        pr_err("ttyssh: Error registering dynamic pipe device: %d\n",
               rv);
    }
...
static int ttyssh_clear_intf(int id, void *p, void *dummy)
{
    struct ttyssh_intf *intf = p;
...
static void __exit ttyssh_exit(void)
{
    misc_deregister(&ttyssh_pipe_misc);
    misc_deregister(&ttyssh_echo_misc);
    idr_for_each(&ttyssh_echo_nums, ttyssh_clear_intf, NULL);
    idr_for_each(&ttyssh_pipe_nums, ttyssh_clear_intf, NULL);
    uart_unregister_driver(&serialecho_driver);
    uart_unregister_driver(&serialpipea_driver);
    uart_unregister_driver(&serialpipeb_driver);

    pr_info("ttyssh unloaded\n");
}

module_init(ttyssh_init);
module_exit(ttyssh_exit);
